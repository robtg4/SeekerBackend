const computeMintedSignature = async (tokenId, recipientAddress, issuingAddress, web3) => {
  ...
  const signature = await web3.eth.signPromise(issuingAddress, hash);
...
const computeNonceSignature = async (nonce, issuingAddress, web3) => {
...
const computeLevelSignature = async (tokenId, tokenLevel, recipientAddress, issuingAddress, web3) => {
  const idString = (new web3.BigNumber(tokenId)).toString(16)
